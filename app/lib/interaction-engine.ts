...
export class InteractionEngine {
  ...
  /** Scales grab/near-mouth proximity radii. 1 = original behavior. Set via setSensitivity(). */
  ...
  /** Scales how close a hand/mouth needs to be to grab or attach the cigarette. 1 = original behavior. */
  setSensitivity(value: number) {
    this.sensitivity = clamp(value, 0.5, 1.75);
  }
  ...
  private updateCigarette(face: FaceAnalysis, hands: HandAnalysis[], now: number, dt: number) {
    ...
    if (state === "IDLE") {
      ...
      const grabRadius = Math.max(0.035, this.snapshot.cigaretteBaseLength * 0.54) * this.sensitivity;
    ...
    const grabber = hands
      .filter((hand) => hand.state !== "NONE")
      .find((hand) => distance(hand.gripPoint, this.snapshot.cigarettePosition) < Math.max(0.035, this.snapshot.cigaretteBaseLength * 0.6) * this.sensitivity);
    if (grabber) {
  ...
  private releaseFromHand(face: FaceAnalysis, allowMouthAttach: boolean) {
    const nearMouth = distance(this.snapshot.cigarettePosition, face.mouthCenter)
      < Math.max(face.mouthWidth * 0.9, this.snapshot.cigaretteBaseLength * 0.72) * this.sensitivity;
