...
export class VisionTracker {
  ...
  /** 1 = every opportunity (HIGH), 2 = every other (MEDIUM), 3 = every third (LOW). */
  private frameSkipInterval = 1;
  private frameSkipCounter = 0;

  /**
   * Throttle how often processLatest() actually runs MediaPipe inference,
   * independent of the renderer's particle-quality tier. This reduces
   * MediaPipe's CPU/GPU load on weaker devices without changing model
   * accuracy or confidence thresholds — just how often it's asked to run.
   */
  setPerformanceMode(mode: "HIGH" | "MEDIUM" | "LOW") {
    this.frameSkipInterval = mode === "HIGH" ? 1 : mode === "MEDIUM" ? 2 : 3;
  }
  ...
  processLatest(video: HTMLVideoElement, sourceTimestamp: number): TrackingFrame | null {
    ...
    // Throttle: only run inference on every Nth genuinely-new frame, where N
    // comes from setPerformanceMode(). This only counts real new frames (not
    // the early-returns above), so it scales inference frequency down
    // predictably regardless of the camera's actual frame rate.
    this.frameSkipCounter += 1;
    if (this.frameSkipCounter < this.frameSkipInterval) return null;
