...
export function SmokingExperience() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const applySettings = (settings: ReturnType<typeof useSettingsStore.getState>) => {
        visual?.setPreset(settings.preset);
        visual?.setIntensity(settings.intensity);
        visual?.setWind(1, 0, settings.windStrength);
        engine?.setSensitivity(settings.sensitivity);
      };
      applySettings(useSettingsStore.getState());
      unsubscribeSettings = useSettingsStore.subscribe(applySettings);
    } catch (error) {
      useInteractionStore.getState().updateRuntime({
    ...
    const startCameraAndTracking = async () => {
      try {
        ...
        await video.play();
        setCameraState("initializing-tracking");
        resize();
      } catch {
  ...
  return (
    <main className="experience" data-camera={cameraState} data-mirrored={mirrored}>
      <video ref={videoRef} className="camera" aria-label="Webcam view" autoPlay muted playsInline />
      ...
      <p className="sr-only" aria-live="polite">
        {cameraState === "ready"
          ? "Virtual cigarette interaction is active."
          : cameraState === "initializing-tracking"
            ? "Camera ready. Preparing hand and face tracking."
            : cameraState === "denied"
